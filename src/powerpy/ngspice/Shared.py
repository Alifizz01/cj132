...
from .psp.EnumFactory import EnumFactory
from .psp.Unit import u_V, u_A, u_s, u_Hz, u_F  # ok

from .SimulationType import SIMULATION_TYPE  # ok
...
class NgSpiceShared:
    ...
    def setup_platform(cls):
        ...
        libpath = ngspice_path.joinpath('dll-vs', 'ngspice{}.dll')
        cls.LIBRARY_PATH = str(libpath)
        scripts_path = ngspice_path.joinpath("share", "ngspice")
        os.environ["SPICE_SCRIPTS"] = str(scripts_path)
...
# === MISSING: lines 454-454 not photographed ===
        self._stderr = []
        self._error_in_stdout = None
        self._error_in_stderr = None
    ...
    def _load_library(self, verbose):
        ...
        #os.environ['SPICE_LIB_DIR'] = "dll//share//ngspice"  # str(Path(self.NGSPICE_PATH).joinpath('share', 'ngspice'))
        ngspice_path = Path(__file__).parent.joinpath('Spice64_dll')
        ngpath = ngspice_path.joinpath('share', 'ngspice')
        os.environ['SPICE_LIB_DIR'] = str(ngpath)
    ...
    def _init_ngspice(self, send_data):
        ...
        self._self_c = self._c  # To prevent garbage collection
    ...
    @staticmethod
    def _exit(exit_status, immediate_unloading, quit_exit, ngspice_id, user_data):
        """Callback for asking for a reaction after controlled exit"""
        self = ffi.from_handle(user_data)
        self._logger.debug('ngspice-id-{} exit status={} immediate_unloding={} quit_exit={}'.format(
            ngspice_id,
            exit_status,
            immediate_unloading,
            quit_exit))
    ...
    def _send_data(data, number_of_vectors, ngspice_id, user_data):
        ...
        self = ffi.from_handle(user_data)
        # self._logger.debug('ngspice-id-{} send_data [{}]'.format(ngspice_id, data.vecindex))
    ...
    def _send_init_data(data, ngspice_id, user_data):
        ...
        #     self._logger.debug('ngspice-id-{} send_init_data'.format(ngspice_id))
        #     number_of_vectors = data.veccount
        #     for i in range(number_of_vectors):
        #         self._logger.debug('  Vector: ' + ffi_string_utf8(data.vecs[i].vecname))
        return self.send_init_data(data, ngspice_id)
    ...
    def _background_thread_running(is_running, ngspice_id, user_data):
        ...
        self = ffi.from_handle(user_data)
        self._logger.debug('ngspice-id-{} background_thread_running {}'.format(ngspice_id, is_running))
    ...
    def get_vsrc_data(self, voltage, time, node, ngspice_id):
        ...
        self._logger.debug('ngspice-id-{} get_vsrc_data @{} node {}'.format(ngspice_id, time, node))
        return 0
    ...
    def get_isrc_data(self, current, time, node, ngspice_id):
        ...
        self._logger.debug('ngspice-id-{} get_isrc_data @{} node {}'.format(ngspice_id, time, node))
        return 0
    ...
    def reset(self):
        ...
        re-run from it?s initial state, overriding the affect of any set or alter commands.  # OCR_UNCLEAR: it?s (apostrophe vs '?'); using ? as placeholder per instructions
